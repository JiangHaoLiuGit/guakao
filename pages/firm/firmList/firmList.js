...
Page({
  ...
  goBtn:function(e){
    ...
    ajax.get(wx.getStorageSync("config").getInfoBy_url,{
      ...
      if(res.data == null){
        ...
        this.data.firm_id = res.data.driverId
        this.setData({
          point:true,
          codeId:res.data.id,
          code:res.data.verificationCode,
          firm_id:res.data.driverId,
        })
      }else{
  ...
  touchC:function(e){
    ...
    this.setData({
      ...
      //点击查看失败原因
      ajax.get(wx.getStorageSync("config").Audit_url,{
        driverId:id,
        auditState:2
      }).then(res => {
        console.log(res)
  ...
  onLoad: function (options) {
    ...
    this.data.open = userInfoTwo.userNo
  ...
  onShareAppMessage: function (e) {
    ...
    return {
      title: "企业注册",
      desc:"马上去企业注册",
      path:'/pages/user/code/code?firm_id=' + this.data.firm_id + "&&codeId=" + this.data.codeId + "&&code=" +  this.data.code + "&&open=" + this.data.open
    }
